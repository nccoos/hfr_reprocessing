...
"""Rename old (2003-2004) CSS files from 'CSS SITE YY:MM:DD HHMM' to CSS_SITE_YY_MM_DD_HHMM.cs4

Usage:
  rename_old_css_fn.py [DIR] [SITE]
Examples:
  rename_old_css_fn.py /Volumes/PASSPORT/Spectra/Haty/css/2004 HATY

Options:
  DIR   Data directory to process [e.g. /Volumes/PASSPORT/Spectra/Haty/css/2004]
  SITE  Site name [e.g. HATY]
...
def filt_datetime(input_string, pattern=None):
    ...
    With 2-digit year pattern
    
    Default pattern follows the template, YY(-)MM(-)DD(-)(hh(:)(mm(:)(ss)))
    with minimum of YY MM and DD (date) supplied in descending order to 
    return its datetime object, otherwise returns None.
    Typical matches include, YYMMDD-hhmmss, YY-MM-DD-hh:mm:ss All
    ...
    filt_datetime('CSS HATY 04:01:01 0230')
   """
