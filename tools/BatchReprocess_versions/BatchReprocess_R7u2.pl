...
#  v3.0 Major revisions from v2.0 by Hardik Parikh. Read notes below. Compatible with R7u2 only.
#
...
########################################################################
	...
	my $execStr = "$procToolsFolder/SpectraArchiver 999 \"$CosProcFolder/\" \"$CosSpectraOutFolder\" \"\" \"\" \"\" 1 \"$configsFolder/\" $doDopplerInterp 0 $SAmove";
	my $output = `$execStr`;
...
sub help {
	print "\n
  BatchReprocess_R7u2.pl  v 3.0  2014-07-17 (HP)
   
  ***************************** Note ******************************
  -	This new script works with Release 7 update 2 tools only. Will not work with Release 6 or older Release 7 update tools. 
  ...
  -	Elliptical processing now possible in this version 3.0. Elliptical License key required. 
  *****************************************************************
  BatchReprocess_R7u2.pl available command line options:
  -h help
  ...
  e.g.
  > BatchReprocess_R7u2.pl -s /Codar/SeaSonde/Archives/Spectra -b CSS_CBBT -1 \"2007/08/22 21:00\" -R 1 -W 1 -S 0 -E 1\n\n\n"  ;
}
