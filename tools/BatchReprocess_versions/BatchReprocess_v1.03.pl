...
# SH if site id 
@siteID = $fileToProcBegins =~ m{.*_(\w{4})}gi;
$siteID = $siteID[0];

# SH -- can we make this work to output into RadialSites for total processing (?)
$targetFolder = sprintf "%s/Reprocessed_%s_%s", $targetFolder, $siteID, strftime("%y%m%d_%H%M%S", localtime $^T);
...
# SH (2009-08-19) modified so configs are copied to a subfolder in targetFolder
$CpRadialConfigFolder = $targetFolder."/RadialConfigs";
unless (-e $CpRadialConfigFolder) {$out = `mkdir -p $CpRadialConfigFolder`;}

$status = `cp -pR "$configsFolder" "$CpRadialConfigFolder"`;
