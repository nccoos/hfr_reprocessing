...
#
#  BatchReprocess_R7_b3.pl  v 2.0
#
#  v1.x Created by Chad Whelan on 2008-01-10.
#  v2.x Modified v1.x by Hardik Parikh on 2013-01-26 for Release 7 Radial Suite.
#  
#
#	Minimum Requirements:
#		COS Release 7 Radial processing tools.
#  		Mac OS 10.5.x, 10.6.x or 10.7.x.
#	    Intel Only. 
...
$startTime = 0;
$stopTime = 0;
$bUseBragg = 0; #default: use both bragg peaks. 
...
$line = <FID>;	#Line 15: WaveModelSlider
$line =~ s/^\s+//;
@params = split(/\s+/,$line);
$diagWMS = $params[0];
...
$diagFOL,$diagSTE,$diagES,$diagEA,$diagED,$diagEM = 1;
$diagSPE = 0;
}
########################################################################
...
#####Commented this out since checking in the Print summary of output section	 
# if ($doWaves) {
# 	foreach (@logtargets) {print { $_ } "\nProcessing for wave data.\n\n";}
# }
...
## Print Script version to new file in processing folder.
	$reprocessingscriptversion = $CosProcFolder."/ReprocessingScriptVersion.txt"; 
	open (VERSION, "> $reprocessingscriptversion");
	print VERSION "BatchReprocessingScript version 2.0\n";
	close VERSION;
...
@filesToDelete = (	
					...
					#$CosProcFolder."/RadialData.txt",#HP: Same as $CosProcRadFile
...
if ($#cssToProcess >= 0) {
	...
	foreach my $csfile (sort @cssToProcess) {
		...
		######################################################################
   		 # Check for correct CheckForSpectra(11.3.0) and RadialArchiver(11.3.3) Versions and for License Key
    	######################################################################
     ...
     if ($csoutput =~ m/Running CheckForSpectra (\d\d)\.(\d)\.(\d)/)
		...
		{ my $execStr = "$procToolsFolder/SpectraDoubler 10 \"$CosProcFolder/\" \"$CosProcFolder/\" 0 1 $doDopplerInterp $doRangeInterp";
		  my $output = `$execStr`;
		}
		...
		if (-e $CosProcSpectraAver) {rename $CosProcSpectraAver, $CosProcSpectraAver.'.old';} ## HP moved from few lines above to here.
		...
		if ($doRads or $doShortRads) {
			...
			foreach $pattType (@pattsToUse) {
				...
				my $execStr = "$procToolsFolder/SpectraToRadial 0 $patt \"$suffix\" $diagSTR 0 \"$configsFolder/\" \"$CosProcFolder/\" 0 0 0 0 -1";
				my $output = `$execStr`;
				foreach (FLOG){printf { $_ } "$output\n";}
				#HPprint "*****SpectraToRadialOutput is:\n$output\n";
				$didCurrent=1;
				...
				if ($doShortRads) 
				{
					...
					# Usage: RadialFiller <nDiag> <bPattern> <sCfgFold> <sProcFold>
					{
						...
						my $execStr = "$procToolsFolder/RadialFiller 0 $patt \"$configsFolder\" \"$CosProcFolder/\"";
						my $output = `$execStr`;
						foreach (FLOG){printf { $_ } "$output\n";}
				...
				if ($doRads) 
				{
					...
					{  # ----- 1st unless
						...
						my $execStr = "$procToolsFolder/RadialMerger $diagRM $timeOutput \"$CosProcRadListFile$suffix\" \"$configsFolder/\" \"$CosProcFolder/\" 1"; #HP added 1
						my $output = `$execStr`;
						foreach (FLOG){printf { $_ } "$output\n";}
					  unless ($?)
					    {  # ----2nd unless
		...
		my $execStr = "$procToolsFolder/CheckEllipticalSetup 1 \"$CosEllipticalFolder/$CosRadSubFolder{$pattType}/\" \"$CosProcFolder/\" \"$configsFolder/\"";
		my $output = `$execStr`;
		$elpstate=$?;
		foreach (FLOG){printf { $_ } "$output\n";}
        ...
        my $execStr = "$procToolsFolder/SpectraToEllipticals 0 $patt $procChar 0 0 \"$configsFolder/\" \"$CosProcFolder/\" 0 $bUseBragg 0";
        my $output = `$execStr`;
        foreach (FLOG){printf { $_ } "$output\n";}
        $didElliptical=1;
        ## Usage: <nDiag> <nElpType> <sProcFolder> <sDiagFolder>
		my $execStr = "$procToolsFolder/EllipticalsDiagnostic 0 $elpType \"$CosProcFolder/\"";
		my $output = `$execStr`;
		foreach (FLOG){printf { $_ } "$output\n";}
		...
		if ($doShortRads)
		{
			...
			my $execStr = "$procToolsFolder/EllipticalsArchiver $diagEA \"$shortChar\" 0 \"$CosShortEllipticalFolder/$CosRadSubFolder{$pattType}/\" \"$CosProcFolder/\" $outputLLUV 0 \"$configsFolder/\" $patt \"\"";
			my $output = `$execStr`;
			foreach (FLOG){printf { $_ } "$output\n";}
			...
			foreach (FLOG){printf { $_ } "$output\n";}
				...
				$et = ($elpType + 4);
				## Usage: <nDiag> <nElpType> <sProcFolder> <sDiagFolder>				
				my $execStr = "$procToolsFolder/EllipticalsDiagnostic 0 $et \"$CosProcFolder/\"";
				my $output = `$execStr`;
				foreach (FLOG){printf { $_ } "$output\n";}
				## Usage: <nDiag> <cProcChar> <sOwner> <sElpFolder> <sProcFolder> <bLLUV> <bOpen> <sCfgFolder> <nPattern> <sDiagFolder>
		...
		if ($doWaves and $doingCSS) {
			...
			if (-e "$procToolsFolder/SpectraToWaveModel" and not -e "$procToolsFolder/SpectraToWavesModel") {
				...
				my $execStr = "$procToolsFolder/SpectraToWaveModel $diagSTWM \"$configsFolder/\" \"$CosProcFolder/\"";
				my $output = `$execStr`;
				foreach (FLOG){printf { $_ } "$output\n";}
			} else {
				## Usage: <nDiag> <sCfgFolder> <sProcFolder>
			...
			$execStr = "$procToolsFolder/WaveModelSlider $diagWMS 1440 \"\" \"\" \"\" \"\" \"$configsFolder/\" \"$CosProcFolder/\" 1 \"\""; #HP
			$output = `$execStr`;
			foreach (FLOG){printf { $_ } "$output\n";}
			...
			if (($doAllImage != 0) && (-e $CosWaveImageTool))
			{
			    ...
			    # if (($doImageWaveLatest != 0) && ($imageWaveCell1 > 0))
# 			    { 
# 			    $p1 = "-rangecell = 1,$imageWaveCell1 -imagefile=\"${CosImageLatestFolder}/WaveModel_RC${imageWaveCell1}.png\" -plot";
# 			    }
# 			      else {$p1 = "";}
# 			    if (($doImageWaveLatest != 0) && ($imageWaveCell2 > 0))
# 			    { 
# 			      $p2 = "-rangecell = 1,$imageWaveCell2 -imagefile=\"${CosImageLatestFolder}/WaveModel_RC${imageWaveCell2}.png\" -plot";
# 			    }
# 			      else {$p2 = "";}
# 			    
# 			    if (($doImageWaveLatest != 0) && ($imageWaveCell3 > 0))
# 			    { 
# 			      $p3 = "-rangecell = 1,$imageWaveCell3 -imagefile=\"${CosImageLatestFolder}/WaveModel_RC${imageWaveCell3}.png\" -plot";
# 			    }
# 			      else {$p3 = "";}
# 			      
# 			    if ($doImageWaveLatest != 0)
# 			    {   ### Think about wavefile 1 and wavefile 2 reading 2 lines from /waveprocessed.txt file in processing folder. 
# 			    	my $execStr = "$CosWaveImageTool $mainwave1File $mainwave2File $plist -size=600,300 -pad=0,10,30,0 \ -timespan=$imageWaveSpan -depth=16 -multirange=off -axis=1,1 -axis=2,2 -axis=3,3 -axis=4,4\ $p1 $p2 $p3";
# 			    	$output = `$execStr`;
# 			    }
...
########################################################################
	...
	if (($doAllImage != 0) && (-e $CosSpectraImageTool))
	{
	   ...
	   if (-e $CosSPMPrefFile)
	   {
		...
		# if ($doImageSpectraLatest != 0)
# 		{
# 		my $execStr = "$CosSpectraImageTool $plist -plot=map -image=\"${CosImageLatestFolder}/Spectra_PowerMap.png\" $mainspectraFile";;
# 		my $output = `$execStr`;
# 		}
		# if ($doImageSpectraRangeLatest != 0) 
# 		{
# 			if ($imageSpectraRangeCell1 > 0) 
# 				{
# 				my $execStr = "$CosSpectraImageTool $plist -plot=range -point=0,$imageSpectraRangeCell1 -size=600,300 -image=\"${CosImageLatestFolder}/Spectra_RangeSlice_RC${imageSpectraRangeCell1}.png\" $mainspectraFile";
# 				my $output = `$execStr`;
# 				}
# 			if ($imageSpectraRangeCell2 > 0) 
# 				{
# 				my $execStr = "$CosSpectraImageTool $plist -plot=range -point=0,$imageSpectraRangeCell2 -size=600,300 -image=\"${CosImageLatestFolder}/Spectra_RangeSlice_RC${imageSpectraRangeCell2}.png\" $mainspectraFile";
# 				my $output = `$execStr`;
# 				}
# 			if ($imageSpectraRangeCell3 > 0) 
# 				{
# 				my $execStr = "$CosSpectraImageTool $plist -plot=range -point=0,$imageSpectraRangeCell3 -size=600,300 -image=\"${CosImageLatestFolder}/Spectra_RangeSlice_RC${imageSpectraRangeCell3}.png\" $mainspectraFile";
# 				my $output = `$execStr`;
# 				}
# 	    }
...
sub help {
	print "\n
  BatchReprocess_R7.pl  v 2.0  2013-01-26 (HP)
   
  ***************************** Note ******************************
  -	This new script works with Release 7 tools. Will not work with Release 6 tools. 
  -	If using R7,requires a usb License Key for operation and
  ...
  -	Elliptical processing now possible in this version 2.0. Elliptical License key required. 
  *****************************************************************
  BatchReprocess_R7.pl available command line options:
  -h help
  ...
  e.g.
  > BatchReprocess.pl -s /Codar/SeaSonde/Archives/Spectra -b CSS_CBBT -1 \"2007/08/22 21:00\" -R 1 -W 1 -S 0 -E 1\n\n\n"  ;
}
